...
class LoadDataset(Dataset):
    def __init__(self, expression, edge_index, y, index, mask, ids):
        self.expression=torch.from_numpy(expression).float()[index]
        self.edge_index=torch.from_numpy(edge_index).long()
        self.edge_index = torch.cat([self.edge_index, torch.tensor([[4223], [4223]])], dim=-1)
        self.y=torch.from_numpy(y).long()[index]
        # self.adj = torch.from_numpy(data["adj_matrix"]).float()
        self.mask=torch.from_numpy(mask).int()
        self.mask = torch.cat([self.mask, torch.tensor([[0, 0]])], dim=0)
        self.ids=torch.from_numpy(ids).int()
    ...
    def __getitem__(self, idx):
        y = self.y[idx]
        x = self.expression[idx, :, :]  # N* 2
        return Data(x=x, edge_index=self.edge_index, y=y, mask=self.mask, ids=self.ids)
